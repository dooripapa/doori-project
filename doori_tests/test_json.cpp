...
{
    doori::DataStream::Json_value value{1111};
    EXPECT_EQ("1111", value.toString());

    doori::DataStream::Json_value value1{"const char*"};
    EXPECT_EQ("\"const char*\"", value1.toString());

    std::string str{"std::string"};
    doori::DataStream::Json_value value2{str};
    EXPECT_EQ("\"std::string\"", value2.toString());

    doori::DataStream::Json_value value3{"const char*"};
    EXPECT_EQ("\"const char*\"", value3.toString());

    doori::DataStream::Json_value value4{0.1234f};
    EXPECT_EQ("0.1234", value4.toString());

    doori::DataStream::Json_value value5{doori::DataStream::Json_value{2222}};
    EXPECT_EQ("2222", value5.toString());

    doori::DataStream::Json_value value6{ value };
    EXPECT_EQ("1111", value6.toString());

    doori::DataStream::Json_value value7{ true };
    EXPECT_EQ("true", value7.toString());
}

TEST(Json_value, ArrayTest)
{
    doori::DataStream::Json_value value;
    value.append({1111});
    value.append({"name"});
    value.append({0.2f});
    value.append({true});
...
TEST(Json_value, SetTest)
{
    ...
    value.set(doori::DataStream::Json_value{2222});
...
{
    doori::DataStream::Json json, another;
    json["key"] = 1234;
...
{
    doori::DataStream::Json json, sub_json;

    std::string literalString=R"({"1":1111})";
    json.append("1", doori::DataStream::Json_value(1111));
    EXPECT_EQ(literalString, json.serialize());
    json.clear();

    literalString=R"({"2":"2222"})";
    json.append("2", doori::DataStream::Json_value("2222"));
    EXPECT_EQ(literalString, json.serialize());
    json.clear();

    literalString=R"({"3":0.1})";
    json.append("3", doori::DataStream::Json_value(0.1f));
    EXPECT_EQ(literalString, json.serialize());
    json.clear();

    literalString=R"({"4":"name"})";
    json.append("4", doori::DataStream::Json_value("name"));
    EXPECT_EQ(literalString, json.serialize());
    json.clear();

    literalString=R"({"1":1,"2":{"2":"2"}})";
    json.append("1", doori::DataStream::Json_value(1));
    sub_json.append("2","2");
    json["2"]=sub_json;
    EXPECT_EQ(literalString,json.serialize());
    json.clear();

...
namespace doori::Common {
        ...
        protected:
             ...
             * @param cause
             */
            inline void InjectByClient(const string& cause) noexcept;

            template<int N>
            auto InjectByClient(char const(&value)[N]) noexcept -> void;
        ...
        template<int N>
        auto Error::InjectByClient(const char (&value)[N]) noexcept -> void {
            mErrno = -1;
            mStatus = false;
            mCause = std::string(value);
        }

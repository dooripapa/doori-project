...
namespace doori {
    namespace CommunicationMember {
        ...
        void TcpApi::Connect(const std::string& ip, const std::string& port) {
        ...
        void TcpApi::Send(const char *data, uint8_t dataLen) {

            if( !mSocket.IsBitwise(SOCK_STATUS::ESTABLISED) ) {
                InjectByClient("Socket is not Establishing");
                return;
            }

            int nRet = 0;
            errno = 0;
            nRet = send(mSocket.GetFd(), data, dataLen, 0);
            if(nRet == -1) {
                InjectBySystemcall();
                LOG(ERROR, "send error:", Cause() );
                return;
            }
            LOG(DEBUG, "complete to send data! size[", nRet, "]");
        }

        void TcpApi::Recv(char *dataContainer, std::uint8_t dataLen) {
            ...
            int nRet = 0;
            errno = 0;
            nRet = recv(mSocket.GetFd(), dataContainer, dataLen, MSG_WAITALL);
            if(nRet == -1) {
                InjectBySystemcall();
                LOG(ERROR, "recv error:", Cause());
                return;
            }
        ...
        void TcpApi::Reply(const char *data, uint8_t dataLen, char *recvData, uint8_t tilDataSize) {
